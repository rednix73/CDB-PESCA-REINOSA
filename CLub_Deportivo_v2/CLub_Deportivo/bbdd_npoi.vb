    ...
    Public Sub PurgeDeletedFederativas_Xlsx(ruta_bd_excel As String, delFile As String, keys As System.Collections.Generic.HashSet(Of String), tabla_federa_xls As String)
        ...
        Try
            ...
            For r As Integer = lastRow To 1 Step -1
                ...
                Catch
                End Try
                If Not String.IsNullOrEmpty(vNif) AndAlso keys.Contains(bbdd.NormalizeId(vNif)) Then
                    ' En la última fila ShiftRows(r+1, r, -1) no hace nada y la fila no se borraba
                    If r < sheet.LastRowNum Then
            ...
            If deletedCount > 0 Then
                ...
                For Each ln In File.ReadAllLines(delFile)
                    Dim s = ln.Trim()
                    If Not String.IsNullOrEmpty(s) AndAlso Not keys.Contains(bbdd.NormalizeId(s)) Then remaining.Add(s)
                Next
